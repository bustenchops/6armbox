...
hexagon = [(int(center[0] + HEX_RADIUS * np.cos(np.deg2rad(60 * i))),
            int(center[1] + HEX_RADIUS * np.sin(np.deg2rad(60 * i)))) for i in range(6)]
...
while True:
    ...
    for pt in hexagon:
        cv2.line(frame, center, pt, (0, 255, 0), 1)
    ...
    if object_found:
        ...
        last_movement_time = time.time()
    elif time.time() - last_movement_time > MOVEMENT_TIMEOUT:
        tracking = False
        last_position = None
