...
def initialize_camera(resolution=(640, 480), fmt="BGR888"):
    ...
    picam2.start()
    time.sleep(2)  # allow exposure and white balance to settle
    return picam2
...
def main():
    ...
    # initialize KCF tracker on color frame
    tracker = create_kcf_tracker()
    tracker.init(frame, bbox)

    # video writer for color output
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    writer = cv2.VideoWriter('output_kcf.avi', fourcc, 30.0, (WIDTH, HEIGHT), True)
    ...
    while True:
        ...
        if not success:
            # re-detect and re-init on failure
